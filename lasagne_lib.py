...
def get_bench_net(input_var, mask_var, inp_dim, rnn_size, out_size, GRAD_CLIP):
    ...
    h1 = lasagne.layers.GRULayer(l_in, num_units=rnn_size, mask_input=l_mask, grad_clipping=GRAD_CLIP,
                                  hid_init=lasagne.init.GlorotUniform())
    h3 = lasagne.layers.SliceLayer(h1, -1, axis=1)
    h4 = lasagne.layers.DenseLayer(h3, num_units=rnn_size)
    h6 = lasagne.layers.DenseLayer(h4, num_units=out_size, nonlinearity=lasagne.nonlinearities.softmax)
...
def get_rnn_net(input_var, mask_var, inp_dim, rnn_size, out_size, GRAD_CLIP, drop_p):
    ...
    # RNN layers
    hf = lasagne.layers.RecurrentLayer(l_in, num_units=rnn_size, mask_input=l_mask, grad_clipping=GRAD_CLIP,
                                  hid_init=lasagne.init.GlorotUniform())

    hb = lasagne.layers.RecurrentLayer(l_in, num_units=rnn_size, mask_input=l_mask, grad_clipping=GRAD_CLIP,
                                       hid_init=lasagne.init.GlorotUniform(), backwards=True)

    hc = lasagne.layers.ConcatLayer([hf, hb], axis=2)

    hp = lasagne.layers.DropoutLayer(hc, p=drop_p)

    # h3 = lasagne.layers.RecurrentLayer(h2, num_units=rnn_size, mask_input=l_mask, grad_clipping=GRAD_CLIP,
    #                                    hid_init=lasagne.init.GlorotUniform())
    # h4 = lasagne.layers.DropoutLayer(h3, p=drop_p)
    #
    # h5 = lasagne.layers.RecurrentLayer(h4, num_units=rnn_size, mask_input=l_mask, grad_clipping=GRAD_CLIP,
    #                                    hid_init=lasagne.init.GlorotUniform())
    # h6 = lasagne.layers.DropoutLayer(h5, p=drop_p)

    h7 = lasagne.layers.SliceLayer(hp, -1, axis=1)
    h8 = lasagne.layers.DenseLayer(h7, num_units=rnn_size)
    h9 = lasagne.layers.DropoutLayer(h8, p=drop_p)
    h10 = lasagne.layers.DenseLayer(h9, num_units=out_size, nonlinearity=lasagne.nonlinearities.softmax)

    return h10

def get_rrnn_net(input_var, mask_var, inp_dim, rnn_size, out_size, GRAD_CLIP, drop_p, period):
    # Input layer
    l_in = lasagne.layers.InputLayer(shape=(None, None, inp_dim), input_var=input_var)
    ...
    h1 = RRNNLayer(l_in, period=period,time_input=None,
            num_units=rnn_size,
            mask_input=l_mask,
