...
class CFNCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    """Most basic RNN: output = new_state = activation(W * input + U * state + B)."""
    with vs.variable_scope(scope or type(self).__name__):  # "BasicRNNCell"
      with vs.variable_scope("t"):
          t = _linear(args=[state, inputs], output_size=self._num_units, bias=True, bias_start=1)
          t = sigmoid(t)
      with vs.variable_scope("e"):
          e = _linear(args=[state, inputs], output_size=self._num_units, bias=True, bias_start=-1)
          e = sigmoid(e)

      with vs.variable_scope("Candidate"):
          output = t * tanh(state) + e * tanh(_linear(args=[inputs], output_size=self._num_units, bias=False))
    return output, output
# t,e = array_ops.split(1, 2, _linear(args=[state, inputs], output_size=2* self._num_units, bias=True, bias_start=1))

def _linear_cfn(args, output_size, bias, bias_start=0.0, scope=None):
  """Linear map: sum_i(args[i] * W[i]), where W[i] is a variable.
  ...
  with vs.variable_scope(scope or "Linear"):
    matrix = vs.get_variable(
        "Matrix", [total_arg_size, output_size], dtype=dtype, initializer=random_uniform_initializer(minval=-0.07, maxval=0.07))
    if len(args) == 1:
      res = math_ops.matmul(args[0], matrix)
    else:
      res = math_ops.matmul(array_ops.concat(1, args), matrix)
...
class MGUCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    """Most basic RNN: output = new_state = activation(W * input + U * state + B)."""
    with vs.variable_scope(scope or type(self).__name__):  # "BasicRNNCell"
      with vs.variable_scope("f"):
          f = _linear(args=[state, inputs], output_size=self._num_units, bias=True, bias_start=0)
          f = sigmoid(f)
      with vs.variable_scope("Candidate"):
          c = tanh(_linear(args=[f*state, inputs], output_size=self._num_units, bias=True, bias_start=0))
      with vs.variable_scope("Activation"):
          new_h = (1-f)*state + f*c
    return new_h, new_h
