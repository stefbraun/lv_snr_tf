...
class BatchIteratorRandom(object):
    """
    """
    ...
    def flow(self, epoch, h5, max_frame_size=1000, shuffle_type='high_throughput', normalization='epoch',
        ...
        with h5py.File(h5) as hf:
            ...
            np.random.seed(1)
            feats=np.random.standard_normal(feats.shape)
            # labels = np.random.permutation(labels)
            # print('Warning: random label permutation. label csum {}'.format(np.sum(labels[:50])))
            print('Warning: random noise training')
            np.random.seed(epoch)
            ...
            while b < nb_batch:
                ...
                for i, sample in enumerate(batch_feats):
                    ...
                    ctc_labels = np.asarray(batch_labels[i])  # shift +1 because 0=blank in CTC training
